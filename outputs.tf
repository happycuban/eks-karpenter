...
############################# INDIVIDUAL OUTPUTS (for scripts/automation) ###############
        ...
        traefik_dns_managed_by                 = "ExternalDNS will create *.happycuban-example.dk pointing to Traefik LoadBalancer"
...
############################# DNS & CERTIFICATES ########################################
        ...
        Wildcard Certificate                   *.happycuban-example.dk (via cert-manager)

############################# SERVICE URLS ###############################################
        ArgoCD Web UI                          https://argocd.happycuban-example.dk
        Traefik Dashboard                      https://traefik.happycuban-example.dk/dashboard
...
############################# ACCESS & SECURITY #########################################
        ...
        Manager Role                           eks-admin-role
        Developer User                         eks-developer-user
...
############################# USAGE EXAMPLES #############################################
        ...
        kubectl annotate service <service-name> external-dns.alpha.kubernetes.io/hostname=myapp.happycuban-example.dk
