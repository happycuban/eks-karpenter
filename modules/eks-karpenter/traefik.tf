...
resource "kubectl_manifest" "traefik-dashboard" {
  ...
  yaml_body = <<-YAML
    ...
    spec:
      entryPoints:
        ...
        - websecure
      routes:
        - match: Host(`traefik-dashboard.${var.domain_name}`)
          kind: Rule
          middlewares:
  ...
  depends_on = [
    ...
    kubectl_manifest.admin_ip_whitelist
  ]
}
    ...
    spec:
      acme:
        ...
        - dns01:
            route53:
              region: ${var.region}
              # Using Pod Identity - no need for accessKeyID or secretAccessKeySecretRef
  YAML
  depends_on = [
...
# which will create the ${var.subject_alternative_names} DNS record pointing to the load balancer
