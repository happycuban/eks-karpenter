...
###############################################################################
locals {
  # Combine base IPs from module with additional environment-specific IPs
  allowed_ips = concat(var.base_allowed_ips, var.additional_allowed_ips)
}
...
module "eks" {
  ...
  cluster_endpoint_public_access_cidrs = var.restrict_cluster_access ? local.allowed_ips : ["0.0.0.0/0"]
...
resource "helm_release" "karpenter" {
  ...
  # Don't specify credentials - let helm provider use system credentials
